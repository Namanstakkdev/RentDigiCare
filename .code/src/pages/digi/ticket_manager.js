...
class TicketManager extends Component {
  ...
  getPermissionList = async (companyId) => {
    try {
      ...
      console.log("datatat", response.data.permission_type, "datatat");
  ...
  async componentDidMount() {
    ...
    this.getPermissionList(userData.companyId);
    ...
    if (userData.role == "customer" || userData.role == "manager") {
      try {
        ...
        if (userData.role == "customer") {
          this.setState({
            propertyID: response.data.property[0]._id,
            property: response.data.property[0].title,
            companyID: response.data.property[0].companyID,
          });
  ...
  toggleTabProgress(tab) {
    ...
    if (!this.state.phone) {
      this.setState({ phoneError: "Please type your phone#" });
    }
  ...
  submitTicketInformation = async () => {
    ...
    try {
      ...
      if (submited) {
        ...
        setTimeout(function () {
          window.location.href = "/"; // Replace "otherpage.html" with the URL of the other page you want to navigate to
        }, 5000);
      }
