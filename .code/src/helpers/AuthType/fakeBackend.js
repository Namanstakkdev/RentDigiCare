import axios2 from "../../pages/api/axios"
...
async function loginMainUsers(user) {
  return axios2.post("login", {
    email: user.email,
    password: user.password,

  }).then((res) => {
    console.log("res.data", res.data.jwtInformation)
    users.push({
      uid: 1,
      role: jwt_decode(res.data.accessToken).role,
      email: user.email,
      accessToken: res.data.accessToken,
      userData: res.data.jwtInformation
    })
    return new Promise(async (resolve, reject) => {
      setTimeout(() => {
        if (res.data.accessToken) {
...
const fakeBackend = () => {
  ...
  mock.onPost("/post-fake-login").reply(config => {
    const user = JSON.parse(config["data"])
    if (window.localStorage.getItem("login_role") === "admin") {
      return loginAdmin(user)
    } else if (window.localStorage.getItem("login_role") === "other-users") {
      return loginMainUsers(user)
    }
  })
