    ...
    if (findSlot.length > 0) {
      findSlot.forEach(async (slot, index) => {
        ...
        if (slot.assignedToType == "manager") {
          ...
          console.log({ userData }, "Cpmpany");
          EmailData.applicantName =
            userData?.ownerName !== undefined
              ? userData.ownerName
              : userData.ownerFirstName !== undefined &&
                userData.ownerLastName !== undefined
...
exports.sendMailforAppointment_twelve_hour = async () => {
  try {
    ...
    console.log(currentTimestamp, oneHourFromNow);
    const findSlot = await User_appointment.find({
      startTimeEpoch: {
        $gte: currentTimestamp,
        $lte: oneHourFromNow,
...
async function addManagerEmails(resultWithManagers) {
  try {
    const managerIds = resultWithManagers
      .map((result) => result.managers)
      .flat();
    const managers = await propertyManager.find(
      { _id: { $in: managerIds } },
...
exports.sentApplicationReportToCompanyUsers = async () => {
  try {
    ...
    for (const application of result) {
      const property = await propertyManager
        .findOne({ properties: application._id })
        .populate("properties");
      if (property) {
        const updatedProperty = {
          ...property.toObject(),
          PropertyID: application._id,
    ...
    for (const item of resultArray) {
      ...
      const category = categorizedResult.find(
    ...
    for (const category of categorizedResult) {
      ...
      const emails = await Company.findOne(
        { _id: category.companyID },
        { scriptContent: 0 }
      );
      if (emails) {
