        ...
        .then((user) => {
          ...
          user
            ...
            .then(async (user) => {
              // send email
              let link =
                `${process.env.DOMAIN_FORGOT_PASSWORD}` +
                "/reset_password?tkn=" +
              ...
              try {
                ...
                const options = await getResetLinkEmailOptions(
                  role,
                  user,
                  link
                );
...
async function getUserDatabase(filter, value) {
  if (filter === "email") {
    ...
    if (await Technical_Staff.exists({ email: value }))
      return [Technical_Staff, "technical_staff"];
  ...
  if (filter === "token") {
    if (await Admin.exists({ resetPasswordToken: value }))
      return [Admin, "admin"];
    if (await Company.exists({ resetPasswordToken: value }))
      return [Company, "company"];
    if (await Technical_Staff.exists({ resetPasswordToken: value }))
      return [Technical_Staff, "technical_staff"];
    if (await Manager.exists({ resetPasswordToken: value }))
      return [Manager, "manager"];
    if (await Customer.exists({ resetPasswordToken: value }))
      return [Customer, "customer"];
    if (await Vendor.exists({ resetPasswordToken: value }))
...
async function getResetLinkEmailOptions(role, user, link) {
  ...
  if (role === "manager") {
    ...
    return Email.generateOptions(
      user.email,
      "PROPERTY_MANAGER_RESET_PASSWORD",
      data
    );
  }
...
router.post("/verify-otp", async (req, res) => {
  if (!req.body.otp) {
    ...
    if (database) {
      database.findOne({ otp: otp, email: req.body.email }).then((user) => {
        if (!user) {
          return res.json({
            status: 401,
...
router.post("/change-password", async (req, res) => {
  if (!req.body.password) {
    ...
    if (database) {
      database.findOne({ email: req.body.email }).then((user) => {
        if (!user) {
          return res.json({
            status: 401,
