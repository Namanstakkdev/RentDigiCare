...
router.get("/dashboardfilter3", async (req, res) => {
  try {
    const {
      role,
      domain,
    ...
    if (searchCriteria.assignedTo === undefined) {
      searchCriteria.assignedTo = { $exists: true, $ne: null, $ne: "" };
    }

    searchCriteria.assignedTo.$type = "objectId";
...
router.get("/dashboardfilter4", async (req, res) => {
  try {
    const {
      role,
      domain,
    ...
    if (searchCriteria.assignSpecificVendors === undefined) {
      searchCriteria.assignSpecificVendors = {
        $exists: true,
        $ne: null,
        $ne: [],
      };
    ...
    searchCriteria.assignSpecificVendors.$elemMatch = { $type: "objectId" };
...
router.post("/filter_tickets_company", authToken, async (req, res) => {
  if (req.body.companyDomain !== undefined) {
    try {
      ...
      const applicationTotal = totalFilterResult.reduce((acc, count) => {
        acc[count._id] = { count: count.count, totalTime: count.totalTime };
        acc["Total"] = (acc["Total"] || 0) + count.count;
        acc["TotalTime"] = (acc["TotalTime"] || 0) + count.totalTime;
        return acc;
      }, {});
