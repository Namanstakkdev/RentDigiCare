...
router.post(
  ...
  /*authToken,*/ async (req, res) => {
    try {
      const editRequestType = await CompanyRequestType.findOneAndUpdate(
        { _id: req.body.id },
        {
          request_type: req.body.request_type,
        }
...
router.post(
  ...
  /*authToken,*/ async (req, res) => {
    try {
      const deleted = await CompanyRequestType.find({
        _id: req.body.id,
      }).remove();
      res.json({
        status: 200,
...
router.post(
  ...
  /*authToken,*/ async (req, res) => {
    try {
      const requestTypes = await CompanyRequestType.find({
        company_id: req.body.company_id,
      });
      res.json({
        status: 200,
