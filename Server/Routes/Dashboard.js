...
router.get("/getallcharts", async (req, res) => {
  try {
      ...
      return new Promise(async (resolve, reject) => {
        try {
          ...
          if (req.query.filterBy === "barChart") {
            ...
            if (role == "manager") {
              matchConditions.eventAssignedTo = ObjectId(
      ...
      return new Promise(async (resolve, reject) => {
        try {
          ...
          if (req.query.filterBy === "barChart") {
            ...
            async function createDynamicAggregationPipeline(
              collection,
              searchFilter
            ) {
              let requestTypes = await getRequestTypes(
                collection,
                searchFilter
              );
              ...
              pipeline = pipeline.concat(
                createAggregationPipeline(requestTypes)
              );
              return pipeline;
            }
            ...
            if (propertyManagerID !== null && req.query.role === "company") {
              ...
              if (propertyID) {
                ...
                filteredDate = filteredDate.filter(
                  (v) =>
                    v.propertyName === propertyName.title.replaceAll(" ", "-")
                );
              }
    ...
    const [appointments, applicants, maintenanceRequests, properties, leads] =
      await Promise.all([
        getAppointments(req),
        getApplicants(req),
